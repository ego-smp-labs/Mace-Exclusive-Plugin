...
public final class AbilityService {
    ...
    private final Map<ExclusiveItemId, List<ActiveAbility>> activeAbilities = new EnumMap<>(ExclusiveItemId.class);
    private final Map<ExclusiveItemId, List<PassiveAbility>> passiveAbilities = new EnumMap<>(ExclusiveItemId.class);

    public AbilityService(ConfigManager configManager, MaceManager maceManager) {
    ...
    public void registerActive(ActiveAbility ability) {
        activeAbilities.computeIfAbsent(ability.weaponId(), ignored -> new ArrayList<>()).add(ability);
    }

    public void registerPassive(PassiveAbility ability) {
        passiveAbilities.computeIfAbsent(ability.weaponId(), ignored -> new ArrayList<>()).add(ability);
    }
    ...
    public void handleInteract(PlayerInteractEvent event) {
        ...
        Optional<ExclusiveItemId> weaponId = weaponId(weapon);
        if (weaponId.isEmpty()) {
            return;
        }
    ...
    public void handleAttack(EntityDamageByEntityEvent event) {
        ...
        for (PassiveAbility ability : passiveAbilities.getOrDefault(weaponId.get(), List.of())) {
            ability.onAttack(context, event);
        }
    }
    ...
    public void handleDamaged(EntityDamageByEntityEvent event) {
        ...
        for (PassiveAbility ability : passiveAbilities.getOrDefault(weaponId.get(), List.of())) {
            ability.onDamaged(context, event);
        }
    }
    ...
    private Optional<ExclusiveItemId> weaponId(ItemStack item) {
        MaceType type = maceManager.getMaceType(item);
        return ExclusiveItemId.fromMaceType(type);
    }
