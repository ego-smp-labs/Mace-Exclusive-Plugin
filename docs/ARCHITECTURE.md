...
5. **Ability Engine (`ability/`)**:
   - Manages custom logic for exclusive skills: `Stored Momentum`, `Ground Pulse` (Power Mace); `Rage State`, `Lunatic Buff` (Chaos Mace); `Void Devourer`, `Abyss Resurrection` (Void Mace); `Siphon Boost` (Vampiric Mace); `Gravity Well` (Gravity Mace); `Sonic Boom` (Warden Spear / Sonic Spear); `Fire Storm` (Soulfire Mace).
